...
--#2: The reporting mart is at a daily grain, so if a listing has multiple amenity changes on the same day, use the latest change from that day.
--#2B: Amenities are parsed/trimmed here; invalid JSON stays NULL rather than an empty array.
    daily_changes AS (
        SELECT
...
--#3B: Valid_to is inclusive. Consecutive change dates produce a one-day window (valid_from = valid_to), which is valid.
    spanned AS (
        SELECT
            listing_id,
            amenities,
